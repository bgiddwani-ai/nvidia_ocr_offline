...
from nv_ocr.utils.image_utils import get_image_by_fitz_doc, fetch_image, smart_resize
from nv_ocr.utils.doc_utils import fitz_doc_to_image, load_images_from_pdf
...
class DotsOCRParser:
    """
    parse image or pdf file, or directory of images
    """
    ...
    def parse_directory(self, input_dir, output_dir="", prompt_mode="prompt_layout_all_en", bbox=None, fitz_preprocess=False):
        """
        Parse all images and PDF files in a directory
        """
        ...
        if total_files == 0:
            raise ValueError(f"No supported files found in directory {input_dir}. Supported formats: {supported_image_extensions + ['.pdf', '.PDF']}")
        
        print(f"Found {len(image_files)} image files and {len(pdf_files)} PDF files in {input_dir}")
        ...
        for pdf_path in pdf_files:
            ...
            pdf_tasks.append({
                ...
                'filename': filename,
                'prompt_mode': prompt_mode,
                'save_dir': pdf_output_dir,
                'file_type': 'pdf'
            })
        
        def _execute_file_task(task):
            if task['file_type'] == 'image':
                return self.parse_image(
                    ...
                    task['save_dir'], 
                    bbox=task['bbox'], 
                    fitz_preprocess=task['fitz_preprocess']
                )
            else:  # PDF
        ...
        if self.use_hf:
            ...
            num_thread = min(total_files, self.num_thread)
        
        print(f"Processing {total_files} files using {num_thread} threads...")
        
        all_results = []
        all_tasks = image_tasks + pdf_tasks
        
        with ThreadPool(num_thread) as pool:
            with tqdm(total=total_files, desc="Processing files") as pbar:
                for results in pool.imap_unordered(_execute_file_task, all_tasks):
                    all_results.extend(results)
                    pbar.update(1)
        
        # Save summary results
        print(f"Parsing finished, results saved to {output_dir}")
        summary_file = os.path.join(output_dir, 'directory_results.jsonl')
    ...
    def parse_file(self, 
        ...
        if os.path.isdir(input_path):
            return self.parse_directory(input_path, output_dir, prompt_mode, bbox, fitz_preprocess)
...
def main():
    ...
    parser.add_argument(
        "input_path", type=str,
        help="Input PDF/image file path or directory containing images"
    )
    ...
    result = nv_ocr_parser.parse_file(
        args.input_path, 
